...
export default function MumbaiSEODetail() {
  ...
  const parsedContent = useMemo(() => {
    ...
    if (typeof DOMParser === "undefined") {
      const content = seoPage.content.rendered;
      ...
      return { heroText, quickAnswerText, tableData: [] };
    }
  ...
  const locationInfo = useMemo(() => {
    // 1. Master branch configuration lookup
    ...
    // 2. Check if parsedContent tableData explicitly has "Nearest Branch" or "Branch" or "Address"
    let detectedBranch = locationDefaults.branchName;

    if (parsedContent?.tableData && parsedContent.tableData.length > 0) {
      const branchRow = parsedContent.tableData.find(row =>
        /nearest.*branch|amigo.*branch|^branch$/i.test(row.label)
      );
      if (branchRow && branchRow.value) {
        const val = branchRow.value.toLowerCase();
        if (val.includes("andheri")) detectedBranch = "Andheri";
        else if (val.includes("thane")) detectedBranch = "Thane";
        else if (val.includes("ghatkopar")) detectedBranch = "Ghatkopar";
      } else {
        const addressRow = parsedContent.tableData.find(row => /address/i.test(row.label));
        if (addressRow && addressRow.value) {
          const val = addressRow.value.toLowerCase();
          if (val.includes("andheri")) detectedBranch = "Andheri";
          else if (val.includes("thane")) detectedBranch = "Thane";
          else if (val.includes("ghatkopar")) detectedBranch = "Ghatkopar";
        }
    ...
    const activeBranchConfig = BRANCH_CONFIGS[detectedBranch] || BRANCH_CONFIGS[locationDefaults.branchName] || BRANCH_CONFIGS.Ghatkopar;

    return {
      area: locationDefaults.area,
      branchName: activeBranchConfig.name,
      branchAddress: activeBranchConfig.address,
      branchMapSrc: activeBranchConfig.mapSrc,
      branchMapHref: activeBranchConfig.mapHref,
      branchLink: activeBranchConfig.link,
      branchPhone: activeBranchConfig.phone,
      heroText: parsedContent?.heroText || `Amigo Academy's ${activeBranchConfig.name} centre is easily accessible from ${locationDefaults.area}. Government certified aviation training and industry-oriented AI & Data Science courses with practical training and placement support.`,
      quickAnswerText: parsedContent?.quickAnswerText || `The nearest Amigo Academy centre is our ${activeBranchConfig.name} branch, convenient for students from ${locationDefaults.area}.`,
      ...
      cabinCrewDesc: parsedContent?.cabinCrewDesc || `Students from ${locationDefaults.area} take our Air Hostess / Cabin Crew course at the ${activeBranchConfig.name} centre. It runs six months and is Maharashtra Government Certified, covering grooming, communication, in-flight service, safety procedures and interview preparation.`,
        ...
        ...(parsedContent?.faqData && parsedContent.faqData.length > 0 ? parsedContent.faqData : [
          {
            ...
            answer: `Amigo Academy does not have a centre directly in ${locationDefaults.area}. Our nearest branch is ${activeBranchConfig.name}, and that is where students from ${locationDefaults.area} attend.`
          },
          {
            question: `How long does it take to reach the ${activeBranchConfig.name} branch from ${locationDefaults.area}?`,
            answer: `Our ${activeBranchConfig.name} centre is conveniently connected from ${locationDefaults.area} by train, bus, or road.`
          },
          {
            question: `Can I attend classes if I work in ${locationDefaults.area} during the day?`,
            answer: `Yes. Our ${activeBranchConfig.name} branch runs morning, afternoon and evening batches. Classes are two hours a day, five days a week, so an evening batch works alongside a job or another course. Speak to a counsellor about which batch has seats.`
          }
        ]),
        {
          question: "What is the height requirement for cabin crew?",
          answer: "Airline cabin crew requirements vary by airline and by recruitment drive. Some airlines specify a minimum height; others use reach or other physical criteria. Amigo Academy's admission criteria are separate from an airline's final hiring criteria — we give you an honest assessment during counselling and tell you which airline requirements to target. Our cabin crew height and eligibility guide has the current detail."
        },
        {
          question: "What salary can I expect after training?",
          answer: "Starting salaries vary considerably by employer, role, location and experience, and differ across the three programmes. Our salary guides carry current indicative ranges for cabin crew, airport ground staff and AI & Data Science roles."
        },
        {
                ...
                {parsedContent && parsedContent.tableData.length > 0 ? (
                  ...
                  <>
                    <tr className="hover:bg-slate-50">
                      <td className="p-3 bg-slate-50 font-bold text-[#112A46] w-1/3">Nearest Branch</td>
                      <td className="p-3">{locationInfo.branchName} Branch</td>
                    </tr>
                    <tr className="hover:bg-slate-50">
        ...
        <div className="max-w-7xl mx-auto space-y-16">
          ...
          <div className="bg-white rounded-[24px] border border-[#E1E5ED] shadow-sm overflow-hidden grid grid-cols-1 lg:grid-cols-12 gap-6 p-6 sm:p-8 relative">
            ...
            <div className="lg:col-span-8 flex flex-col justify-between text-left space-y-6">
              ...
              <div className="pt-2">
                <button
                  onClick={() => { navigate({ to: "/career-guide/cabin-crew" }); window.scrollTo({ top: 0, behavior: "smooth" }); }}
          ...
          <div className="bg-white rounded-[24px] border border-[#E1E5ED] shadow-sm overflow-hidden grid grid-cols-1 lg:grid-cols-12 gap-6 p-6 sm:p-8 relative">
            ...
            <div className="lg:col-span-8 flex flex-col justify-between text-left space-y-6">
              ...
              <div className="pt-2">
                <button
                  onClick={() => { navigate({ to: "/career-guide/airport-ground-staff" }); window.scrollTo({ top: 0, behavior: "smooth" }); }}
                  className="bg-[#112A46] hover:bg-[#0b1d31] text-white font-sans font-bold text-xs sm:text-sm px-6 py-3 rounded-lg transition-all cursor-pointer"
                >
                  View Syllabus
                </button>
          ...
          <div className="bg-white rounded-[24px] border border-[#E1E5ED] shadow-sm overflow-hidden grid grid-cols-1 lg:grid-cols-12 gap-6 p-6 sm:p-8 relative">
            ...
            <div className="lg:col-span-8 flex flex-col justify-between text-left space-y-6">
              ...
              <div className="pt-2">
                <button
                  onClick={() => { navigate({ to: "/career-guide/ai-data-science" }); window.scrollTo({ top: 0, behavior: "smooth" }); }}
                  className="bg-[#112A46] hover:bg-[#0b1d31] text-white font-sans font-bold text-xs sm:text-sm px-6 py-3 rounded-lg transition-all cursor-pointer"
                >
                  View Syllabus
                </button>
