...
async function getStartManifest(matchedRoutes) {
  var _a;
  const { tsrStartManifest } = await import('./_tanstack-start-manifest_v-C0ZWDHVC.mjs');
...
async function getServerFnById(id, access) {
  ...
  const fnModule = (_a = serverFnInfo.module) != null ? _a : await serverFnInfo.importer();
  if (!fnModule) {
...
async function getCsrfRequestValidationResult(opts, ctx) {
  ...
  if (fetchSite !== null) return matchValue((_a = opts.secFetchSite) != null ? _a : "same-origin", fetchSite, ctx);
...
async function getFailureResponse(opts, ctx) {
  ...
  return (_b = (_a = opts.failureResponse) == null ? void 0 : _a.clone()) != null ? _b : new Response("Forbidden", {
    status: 403
  });
}
  ...
  return await (async () => {
    ...
    try {
      ...
      return new Response(serializedError, {
        status: (_a = response.status) != null ? _a : 500,
...
function getStringAttr(attrs, name, fallbackName) {
  var _a;
  const value = (_a = attrs == null ? void 0 : attrs[name]) != null ? _a : fallbackName ? attrs == null ? void 0 : attrs[fallbackName] : void 0;
...
function collectStaticHintsFromManifest(manifest2, matchedRoutes) {
  ...
  for (const route of matchedRoutes) {
    ...
    for (const link of (_a = routeManifest.preloads) != null ? _a : []) {
    ...
    for (const link of (_b = routeManifest.css) != null ? _b : []) {
      const stylesheetHref = getStylesheetHref(link);
      if (((_c = manifest2.inlineCss) == null ? void 0 : _c.styles[stylesheetHref]) !== void 0) continue;
...
function getStaticHandlerInlineCssDefault(handlerInlineCss) {
  ...
  return handlerInlineCss != null ? handlerInlineCss : true;
}
async function resolveInlineCssForRequest(opts) {
  var _a;
  if (opts.requestInlineCss !== void 0) return opts.requestInlineCss;
  if (typeof opts.handlerInlineCss === "function") return await opts.handlerInlineCss({ request: opts.request });
  return (_a = opts.handlerInlineCss) != null ? _a : true;
}
...
async function loadEntries() {
  const [routerEntry, startEntry, pluginAdapters] = await Promise.all([
    import('./router-CBeBCufh.mjs').then((n) => n.r),
...
async function executeMiddleware(middlewares, ctx, signal) {
  ...
  const disposeStreamResponse = async (reason) => {
    ...
    retiredStreamIdentities != null ? retiredStreamIdentities : retiredStreamIdentities = /* @__PURE__ */ new WeakSet();
  ...
  const startRequestResolver = async (request, requestOpts) => {
    ...
    try {
      ...
      if (SERVER_FN_BASE && url.pathname.startsWith(SERVER_FN_BASE)) {
        ...
        bindSsrResponseToRequest(router != null ? router : void 0, result, request.signal);
        request.signal.throwIfAborted();
      ...
      bindSsrResponseToRequest(router != null ? router : void 0, response, request.signal);
      request.signal.throwIfAborted();
...
async function handleServerRoutes({ getRouter, request, url, executeRouter, context, executedRequestMiddlewares }) {
  ...
  if ((server2 == null ? void 0 : server2.handlers) && isExactMatch) {
    ...
    const handler = requestMethod === "HEAD" ? (_c = (_b = handlers["HEAD"]) != null ? _b : handlers["GET"]) != null ? _c : handlers["ANY"] : (_d = handlers[requestMethod]) != null ? _d : handlers["ANY"];
    isHeadFallback = requestMethod === "HEAD" && handler !== void 0 && !handlers["HEAD"];
    if (handler) {
      const mayDefer = !!foundRoute.options.component;
      if (typeof handler === "function") routeMiddlewares.push(handlerToMiddleware(handler, mayDefer));
      else {
        if ((_e = handler.middleware) == null ? void 0 : _e.length) {
...
const index = eventHandler(async (event) => {
  console.log("Nitro SSR handler hit:", event.path);
  try {
